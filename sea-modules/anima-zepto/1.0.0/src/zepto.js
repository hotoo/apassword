...
//Touch Module
//TODO：Fix Tab penetrate bug on Android
  ...
  $(document).ready(function(){
    ...
    $(document)
      ...
      .on('touchstart MSPointerDown pointerdown', function(e){
        ...
        now = Date.now()
        delta = now - (touch.last || now) //时间差
        touch.el = $('tagName' in firstTouch.target ?
          firstTouch.target : firstTouch.target.parentNode)
        touchTimeout && clearTimeout(touchTimeout)
        touch.x1 = firstTouch.pageX
        touch.y1 = firstTouch.pageY
        if (delta > 0 && delta <= 250) touch.isDoubleTap = true //判断是否双击
        touch.last = now
...
//Thanks to gesture.js: http://gitlab.alibaba-inc.com/mtb/lib-gesture/blob/master/src/gesture.js
//Need optimize
//解决元素绑定范围的问题
;(function($){
      ...
      for(var i = 0 ; i < event.changedTouches.length ; i++ ) { //每一个手指
          ...
          if (gesture.status === 'tapping' && distance > 10) {
              ...
              if(Math.abs(displacementX) > Math.abs(displacementY)) { //水平距离超过垂直 则视为水平拖动
                  fireEvent(gesture.element, 'horizontaldragstart', { //水平拖动start
                      touch: touch,
                      touchEvent: event
                  });
                  gesture.isVertical = false;
              } else {
                  fireEvent(gesture.element, 'verticaldragstart', { //垂直拖动start
                      touch: touch,
                      touchEvent: event
                  });
  ...
  function touchendHandler(event) {
      ...
      for (var i = 0; i < event.changedTouches.length; i++) {
          ...
          if (gesture.status === 'pressing') { //pressing之后再移动 没有考虑
