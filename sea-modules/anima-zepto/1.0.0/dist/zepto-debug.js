define("anima/zepto/1.0.0/zepto-debug", [], function(require, exports, module) {
    ...
    (function($) {
        var doc = window.document, docEl = doc.documentElement, slice = Array.prototype.slice, gestures = {}, lastTap = null;
        ...
        function touchstartHandler(event) {
            ...
            for (var i = 0; i < event.changedTouches.length; i++) {
                //手指循环，单指的touches是1
                var touch = event.changedTouches[i], touchRecord = {};
                for (var p in touch) {
        ...
        function touchmoveHandler(event) {
            for (var i = 0; i < event.changedTouches.length; i++) {
                //每一个手指
                var touch = event.changedTouches[i], gesture = gestures[touch.identifier];
                if (!gesture) {
                    return;
                }
                ...
                //直线距离
                // magic number 10: moving 10px means drag, not tap
                // 移动超过10 视为dragging移动
                if (gesture.status === "tapping" && distance > 10) {
                    gesture.status = "dragging";
                    fireEvent(gesture.element, "dragstart", {
                        touch: touch,
                        touchEvent: event
                    });
                    if (Math.abs(displacementX) > Math.abs(displacementY)) {
                        //水平距离超过垂直 则视为水平拖动
                        fireEvent(gesture.element, "horizontaldragstart", {
                            //水平拖动start
                            touch: touch,
                            touchEvent: event
                        });
                        ...
                        fireEvent(gesture.element, "verticaldragstart", {
                            //垂直拖动start
                            touch: touch,
                            touchEvent: event
                        });
                ...
                if (gesture.status === "dragging") {
                    gesture.panTime = Date.now();
                    fireEvent(gesture.element, "drag", {
                    ...
                    if (gesture.isVertical) {
                        fireEvent(gesture.element, "verticaldrag", {
                            displacementY: displacementY,
                            touch: touch,
                            touchEvent: event
                        });
                    } else {
                        fireEvent(gesture.element, "horizontaldrag", {
                            displacementX: displacementX,
                            touch: touch,
                            touchEvent: event
                        });
            ...
            if (Object.keys(gestures).length == 2) {
                //多指情况
                var position = [], current = [], elements = [], transform;
        ...
        function touchendHandler(event) {
            ...
            for (var i = 0; i < event.changedTouches.length; i++) {
                var touch = event.changedTouches[i], id = touch.identifier, gesture = gestures[id];
                if (!gesture) continue;
                ...
                if (gesture.status === "dragging") {
                    var duration = Date.now() - gesture.startTime, velocityX = (touch.clientX - gesture.startTouch.clientX) / duration, velocityY = (touch.clientY - gesture.startTouch.clientY) / duration, displacementX = touch.clientX - gesture.startTouch.clientX, displacementY = touch.clientY - gesture.startTouch.clientY;
                    var velocity = Math.sqrt(gesture.velocityY * gesture.velocityY + gesture.velocityX * gesture.velocityX);
                    fireEvent(gesture.element, "dragend", {
                        isflick: velocity > .5,
                        touch: touch,
                        touchEvent: event
                    });
        ...
        [ "tap", "doubletap", "press", "pressend", "drag", "dragstart", "dragend" ].forEach(function(eventName) {
