 ...
 */
    var selector_hasDuplicate, matches = docElem.webkitMatchesSelector || docElem.mozMatchesSelector || docElem.oMatchesSelector || docElem.msMatchesSelector, selector_sortOrder = function(a, b) {
 ...
 */
    ...
    (function($) {
        var support = $.support, supportTouch = "ontouchend" in document, supportPointer = navigator.msPointerEnabled;
        $.extend(support, {
            touch: supportTouch,
            pointer: supportPointer
        });
        if (!(supportTouch || supportPointer)) return;
        var touch = {}, touchTimeout, tapTimeout, swipeTimeout, longTapDelay = 750, longTapTimeout, EVENT = supportTouch ? {
            START: "touchstart",
            MOVE: "touchmove",
            END: "touchend",
            CANCEL: "touchcancel"
        ...
        $(function() {
            var now, delta;
            $(document.body).on(EVENT.START, function(e) {
                ...
                if (touch.x2 && Math.abs(touch.x1 - touch.x2) > 30 || touch.y2 && Math.abs(touch.y1 - touch.y2) > 30) swipeTimeout = setTimeout(function() {
                    ...
                    touch = {};
                }, 0); else if ("last" in touch) // delay by one tick so we can cancel the 'tap' event if 'scroll' fires
